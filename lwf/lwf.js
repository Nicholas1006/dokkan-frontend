...
const lwfUrlInput = document.getElementById('lwf-url-input');
const sceneNameInput = document.getElementById('scene-name-input');
let lwfUrl = lwfUrlInput.value;
...
lwfUrlInput.addEventListener('input', (e) => {
    ...
    fetch(url, { method: 'HEAD' })
        .then(response => {
            if (response.ok) {
                document.getElementById('valid-lwf').style.backgroundColor = 'green';
                lwfUrl = url;
                settings.lwf = url;
...
lwfUrlInput.dispatchEvent(new Event('input', { bubbles: true, cancelable: true }));

sceneNameInput.addEventListener('input', (e) => {
    document.getElementById('valid-lwf').style.backgroundColor = 'yellow';
    try {
        ...
        console.error('Error loading LWF:', error);
        document.getElementById('valid-lwf').style.backgroundColor = 'red';
    }
});
